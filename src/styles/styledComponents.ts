...
export const MenuItem: ComponentStyleConfig = {
  baseStyle: (props) => ({
    display: "flex",
    alignItems: "center",
    fontWeight: 500,
...
export const AccountCard: ComponentStyleConfig = {
  baseStyle: (props) => {
    const { colorScheme: c } = props;
    const hexValue = theme?.colors?.[c];

    return {
      display: 'flex',
      background: c ? `${c}.300` : mode('white', 'black')(props),
      padding: '1rem',
      gridGap: '0.5rem',
      borderRadius: '0.5rem',
      ...
      '.description': {
        ...
        color: hexValue ? chroma(hexValue[300]).darken().hex() : "gray.400"
      },
      '.iconBox': {
        display: "flex",
        alignItems: "center",
...
export const StatementListItem: ComponentStyleConfig = {
  baseStyle: (props) => {
    return {
      display: 'grid',
